...
Ceph 利用Monitor集群来提高可靠性和容错能力。但是，当使用Monitor集群时，集群中的一个或多个Monitor可能会由于延迟或其他故障而落后。Ceph 通过要求多个Monitor实例就集群状态达成一致来减轻这些负面影响。为了在Monitor之间就集群状态达成共识，Ceph 使用 Paxos 算法和大多数Monitor（例如，一个监视器位于包含1个监视器的集群中；2个监视器位于包含在3个监视器的集群中；3个监视器位于包含5个监视器的集群中；4个监视器位于包含6个监视器的集群中）。
